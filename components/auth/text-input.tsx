    ...
    return (
        <div className="flex flex-col relative w-full">
            <Input
                ...
                {...rest}
                onBlur={focusOut}
                className={`px-[2.1rem] py-[2.7rem] text-[1.6rem] border rounded-[1.2rem] border-[#FAF0CF]/10 bg-transparent text-[#FAF0CF]/80 z-10 outline-none transition-all ${error && 'border-[#dc3939]'} `}
                disabled={disabled}
            />
            <span className={`px-1 text-[#FAF0CF]/50 text-[1.6rem] absolute ${isFocused ? '-top-5 z-20 dark-bg' : 'top-[1.6rem]'} left-[1.9rem] transition-all ${error && 'text-[#dc3939]'}`}>
                {error ? error : label}
            </span>
